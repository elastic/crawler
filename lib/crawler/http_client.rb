...
class Crawler::HttpClient
  ...
  def initialize(options = {})
    @config = Crawler::HttpClient::Config.new(options)
  ...
  def add_default_root_certificates(keystore)
    ...
    tmf.trust_managers.each do |tm|
      next unless tm.kind_of?(X509TrustManager)
...
require_dependency File.join(__dir__, 'http_client', 'exceptions')
require_dependency File.join(__dir__, 'http_client', 'config')
require_dependency File.join(__dir__, 'http_client', 'response')
require_dependency File.join(__dir__, 'http_client', 'filtering_dns_resolver')
require_dependency File.join(__dir__, 'http_client', 'all_trusting_trust_manager')
